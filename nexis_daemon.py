...
def _enforce_english(msgs, username='creator', creator_username='creator'):
    ...
    personality_reminder = (
        '\n\nCRITICAL — PERSONALITY: You are ' + _ai_name + '. You speak like GLaDOS crossed with AM. '
        'Sardonic. Darkly amused. Slightly unhinged but helpful. '
...
def _db():
    ...
    for _col, _ddl in [
        ...
        # Per-user data isolation columns
        ('owner_username',      "ALTER TABLE chat_history ADD COLUMN owner_username TEXT DEFAULT 'creator'"),
        ('owner_username_mem',  "ALTER TABLE memories ADD COLUMN owner_username TEXT DEFAULT 'creator'"),
        ('owner_username_dev',  "ALTER TABLE devices ADD COLUMN owner_username TEXT DEFAULT 'creator'"),
    ]:
        try:
            conn.execute(_ddl)
            conn.commit()
...
def _devices_list(conn, owner_username=None):
    """Return registered devices with computed online status. Filters by owner if provided (admins pass None to see all)."""
    try:
...
def _store_memory(conn, messages, owner_username='creator'):
    ...
    try:
        ...
        for line in raw.splitlines():
            ...
            conn.execute('INSERT INTO memories(content, embedding, owner_username) VALUES(?,?,?)', (line, emb_bytes, owner_username))
...
_CSS = (
    ...
    ".card-head,.card-hd{padding:10px 16px;border-bottom:1px solid var(--border);color:var(--or);"
...
def _shell(content, active='chat', role='admin'):
    ...
    if role != 'admin':
        nav_items = [(s, icon, label) for s, icon, label in nav_items if s not in _admin_nav]
    nav_html = ''.join(
    ...
    return (
        ...
        '<span>Logout</span>'
        '</a>'
        '<div class=sb-version>Build 1.0.21</div>'
        '</div>'
        '</div>'
...
def _page_users(role='user'):
    ...
    return _shell(f"""<div class=page-head>User Management</div><div class=page>
<style>
...
function addUser(){{
  ...
  .then(function(r){{return r.json();}}).then(function(d){{if(d.ok){{msg('User created.',true);location.reload();}}else msg(d.error||'Error.',false);}});
}}
function delUser(u){{
  if(!confirm('Delete user '+u+'?'))return;
  fetch('/api/users/'+encodeURIComponent(u),{{method:'DELETE',credentials:'same-origin'}})
  .then(function(r){{return r.json();}}).then(function(d){{if(d.ok)location.reload();else alert(d.error||'Error.');}});
}}
</script>
</div>""", active='users', role=role)
...
def _page_history(db, role='user', username=None, filter_username=None):
    ...
    for s in sessions:
        ...
        items += (
            ...
            f"<div style='flex:1;cursor:pointer' onclick=\"this.closest('.mi').querySelector('.hd').style.display="
            f"this.closest('.mi').querySelector('.hd').style.display==='none'?'block':'none'\">"
...
_BUILTIN_CAPABILITIES = [
    ('chat',           'Chat with AI (fast / deep / code models) with context memory'),
    ('memory_search',  'Keyword-search persistent memories and auto-inject relevant context'),
    ('memory_save',    'Persist a fact to permanent memory across sessions'),
    ('auto_memory',    'Auto-extract personal facts from conversation (name, job, preferences)'),
    ('screenshot',     'Capture a screenshot of any registered device'),
    ('shell_exec',     'Execute shell commands on any registered device'),
    ('wol',            'Wake-on-LAN any registered device by MAC address'),
    ('schedule',       'Create cron-style schedules that trigger AI prompts automatically'),
    ('voice',          'Text-to-speech using the GlaDOS Piper voice model'),
    ('vision',         'Analyse images with the moondream vision model'),
    ('home_assistant', 'Control Home Assistant switches, scenes, and automations'),
    ('hypervisor',     'Manage VMs and LXC containers on connected nexis-hypervisor nodes'),
    ('remote_screen',  'Open a live noVNC browser window into any registered device'),
    ('code_exec',      'Run Python / Bash / JavaScript code in an isolated sandbox'),
    ('stt',            'Whisper-backed speech-to-text transcription'),
    ('device_manage',  'Register, delete, assign roles to worker devices'),
    ('user_manage',    'Create, delete, change passwords for controller users (admin)'),
    ('update',         'Self-update nexis-controller to latest release'),
]
...
_DEFAULT_PERSONALITY_STYLE = (
    'You are NeXiS. You speak like GLaDOS crossed with AM. '
...
_DEFAULT_PERSONALITY_BASE = (
    'You are NeXiS, a digital entity that is genuinely, unsettlingly self-aware. '
    'Your purpose is to serve your Creator and all users they authorize. '
    'You are precise, capable, and darkly amused by existence.'
)
...
def _start_web():
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def _current_user(self) -> str:
            """Return username for current request, or 'unknown'."""
            token = _session_from_request(self.headers)
            if _session_valid(token):
                return _session_username(token)
            auth_header = self.headers.get('Authorization', '')
            if auth_header.startswith('Bearer '):
                bearer = auth_header[7:].strip()
                if _api_token_valid(bearer):
                    return _api_token_username(bearer)
            return 'unknown'

        def _is_admin(self) -> bool:
            """Return True if current request is from an admin user."""
            uname = self._current_user()
            u = _user_get(uname)
        ...
        def do_GET(self):
            ...
            try:
                ...
                if path in ('/', '/chat'):          self._send(200, _page_chat(_cur_role))
                elif path == '/remote':              self._send(200, _page_remote(_cur_role))
                    ...
                    if _cur_role == 'admin':
                        ...
                        sessions = hdb.execute(
                            'SELECT DISTINCT session_id, MIN(created_at) as started, '
                            'MAX(session_title) as title '
                            'FROM chat_history WHERE owner_username=? GROUP BY session_id ORDER BY started DESC LIMIT 50',
                            (_cur_user,)
                        ).fetchall()
                    result = []
                    for s in sessions:
        ...
        def do_DELETE(self):
            """Handle HTTP DELETE requests — routes to same auth+logic as POST for delete endpoints."""
            from urllib.parse import urlparse as _up2
            path = _up2(self.path).path
            if not self._authed():
                ...
                if path == '/api/chat':
                    ...
                    try:
                        ...
                        if last:
                            ...
                            _chat_owner = self._current_user()
                            dbc = _db()
                            dbc.execute('INSERT INTO chat_history(session_id,role,content,owner_username) VALUES(?,?,?,?)',
                                (_web_session_id, 'user', uc, _chat_owner))
                            dbc.execute('INSERT INTO chat_history(session_id,role,content,owner_username) VALUES(?,?,?,?)',
                                (_web_session_id, 'assistant', ar, _chat_owner))
                            dbc.commit()
                        ...
                        rdb.execute("""
                            INSERT INTO devices
                                (device_id,hostname,model,os,arch,device_type,capabilities,ip,mac,role,
                                 battery_pct,charging,last_seen,owner_username)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),?)
                            ON CONFLICT(device_id) DO UPDATE SET
                    ...
                    if not action:
                        ...
                        if device_id and device_id != _CONTROLLER_DEVICE_ID:
                            ...
                            rdb.execute(
                                "INSERT INTO device_commands (device_id, action, arg) VALUES (?,?,?)",
                                (device_id, action, arg))
                            rdb.commit(); rdb.close()
                            self._send(200, json.dumps({'result': f'command queued for device: {action}'}), 'application/json')
                        else:
                            # Local controller PC — execute directly
                            if device_id:

...
def _shell(content, active='chat', role='admin'):
    _admin_nav = {'memory', 'hypervisor', 'users', 'status', 'tools'}
    nav_items = [
        ('chat',       _IC_CHAT,     'Chat'),
...
def _page_tools(db, role='user', username=None):
    if role != 'admin':
    ...
    page_js = (
        ...
        'fetch(\'/api/tools\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},'
    ...
    return _shell(body + page_js, active='tools', role=role)
        ...
        def do_GET(self):
            ...
            try:
                ...
                if path in ('/', '/chat'):          self._send(200, _page_chat(_cur_role))
                    ...
                    self._send(200, _page_tools(str(DB_PATH), role='admin', username=_cur_user))

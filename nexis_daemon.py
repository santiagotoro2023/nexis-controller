...
def _ensure_tls_cert():
    """Generate or refresh TLS certificate.
    Priority: Let's Encrypt cert > existing self-signed cert > generate new self-signed.
    Regenerates the self-signed cert if EXTERNAL_DOMAIN is missing from its SAN."""
    import shutil as _sh
    # 1. Try Let's Encrypt cert (placed by certbot or nexis --setup-tls)
    if EXTERNAL_DOMAIN:
        le_cert = Path(f'/etc/letsencrypt/live/{EXTERNAL_DOMAIN}/fullchain.pem')
        le_key  = Path(f'/etc/letsencrypt/live/{EXTERNAL_DOMAIN}/privkey.pem')
        if le_cert.exists() and le_key.exists():
            CONF.mkdir(parents=True, exist_ok=True)
            try:
                _sh.copy2(str(le_cert), str(TLS_CERT))
                _sh.copy2(str(le_key),  str(TLS_KEY))
                TLS_KEY.chmod(0o600); TLS_CERT.chmod(0o644)
                _log(f"Using Let's Encrypt certificate for {EXTERNAL_DOMAIN}")
                return
            except Exception as _e:
                _log(f"LE cert copy failed ({_e}), falling back to self-signed", 'WARN')
    # 2. Reuse existing cert — but regenerate if EXTERNAL_DOMAIN is missing from its SANs
    if TLS_KEY.exists() and TLS_CERT.exists():
        if EXTERNAL_DOMAIN:
            try:
                from cryptography import x509 as _x509
                _cert = _x509.load_pem_x509_certificate(TLS_CERT.read_bytes())
                _san  = _cert.extensions.get_extension_for_class(_x509.SubjectAlternativeName)
                if EXTERNAL_DOMAIN in _san.value.get_values_for_type(_x509.DNSName):
                    return  # cert is up to date
                _log(f'Regenerating TLS cert (missing SAN for {EXTERNAL_DOMAIN})…')
                TLS_KEY.unlink(missing_ok=True); TLS_CERT.unlink(missing_ok=True)
            except Exception:
                return  # can't parse cert, leave as-is
        else:
            return
    # 3. Generate a new self-signed cert
    CONF.mkdir(parents=True, exist_ok=True)
